...
namespace GreatWall
{
    ...
    public class DayDialog : IDialog<string>
    {
        ...
        private async Task OnPasswordReceivedAsync(IDialogContext context, IAwaitable<IMessageActivity> argument)
        {
            ...
            {
                // DB에 값을 저장
                using (MySqlConnection connection = new MySqlConnection("Server=localhost;Port=3306;Database=rentroom;Uid=root;Pwd=root"))
                {
                    await connection.OpenAsync();

                    // 방 정보 조회
                    string roomCommand = $"SELECT price, max, addprice FROM room WHERE name = '{strOrder}'";
