...
namespace GreatWall
{
    ...
    public class CHKDialog : IDialog<string>
    {
        ...
        string selectQuery;
        string deleteQuery;
        static DBManager dbManager = new DBManager("localhost", 3306, "rentroom", "root", "root");
                                               ...
                                               IAwaitable<object> result)
        {
            if (result != null)
            {
                ...
                else
                {
                    ...
                    else if (password == null && CHKSelect($"SELECT * FROM reserve where pwd = '{activity.Text.Trim()}' and phone = '{phoneNum}'"))
                    {
