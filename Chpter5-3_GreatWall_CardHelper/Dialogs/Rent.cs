...
namespace GreatWall
{
    ...
    public class RentDialog : IDialog<string>
    {
        ...
        static DBManager DBM = new DBManager("localhost",3306,"rentroom","root","root");
                                               ...
                                               IAwaitable<object> result)
        {
            ...
            if (result != null)
            {
                ...
                {
                    OrderCHK = 0;
                    //db에 해당 정보들 모두 insert
                    //chkMonth = 월 , ChoiceDay = 시작일, rentDays = 몇박, phonNumber=전화번호, password=비번
                    /*
                                        string checkInsert = DBM.insertData(choiceRoom, chkMonth, choiceDay, rentDays, phoneNumber,
                                            password, totalNumber, totalPrice);*/
                    ...
                    $"VALUES('{choiceRoom}', {chkMonth}, {choiceDay}, {rentDays},'{phoneNumber}','{password}',{totalPrice},{totalNumber})"))
                    {
                        ...
                        actions.Add(new CardAction() { Title = "확인", Value = "확인", Type = ActionTypes.ImBack });
                        
                        message.Attachments.Add(                    //Create Hero Card & attachment
                            new HeroCard { Title = "예약정보 확인 되셨으면 확인을 눌러주세요.", Buttons = actions }.ToAttachment()
                        );
